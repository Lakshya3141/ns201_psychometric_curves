...
expName = 'First_try'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\chauh\\Documents\\GitHub\\ns201_psychometric_curves\\First_try_lastrun.py',
...
trials = data.TrialHandler(nReps=2, method='random', 
    ...
    trialList=data.importConditions('bbg_bd.csv'),
    seed=1, name='trials')
...
for thisTrial in trials:
    ...
    # update component parameters for each repeat
    image.setImage(File)
    image_2.setImage(BGbase)
    image_3.setImage(BGbase)
    key_resp.keys = []
    key_resp.rt = []
    _key_resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [polygon, image, image_2, image_3, up, down, left, right, center, key_resp]
    ...
    while continueRoutine:
        ...
        # *image_2* updates
        if image_2.status == NOT_STARTED and tThisFlip >= 0.7-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *up* updates
        if up.status == NOT_STARTED and tThisFlip >= 1.2-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *down* updates
        if down.status == NOT_STARTED and tThisFlip >= 1.2-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *left* updates
        if left.status == NOT_STARTED and tThisFlip >= 1.2-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *right* updates
        if right.status == NOT_STARTED and tThisFlip >= 1.2-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *center* updates
        if center.status == NOT_STARTED and tThisFlip >= 1.2-frameTolerance:
            # keep track of start time/frame for later
        ...
        if key_resp.status == STARTED and not waitOnFlip:
            theseKeys = key_resp.getKeys(keyList=['left', 'right', 'up', 'down', 'a'], waitRelease=False)
